  ...
  return (
    <footer className='relative overflow-hidden bg-black '>
      <div
        ...
        bg-[radial-gradient(55%_60%_at_70%_35%,rgba(13,88,202,0.35)_0%,transparent_70%)]"
      />

       <div
        className="pointer-events-none absolute inset-0 
        bg-[radial-gradient(50%_55%_at_30%_70%,rgba(16,185,129,0.30)_0%,transparent_70%)]"
      />
      ...
      >
        ...
        <div className="h-[3px] w-24 md:w-32 rounded-full bg-gradient-to-r from-[#0d58cc] via-cyan-300 to-emerald-400 "/>
          <div className="flex gap-5 text-2xl md:text-3xl mt-6 ">
            {socials.map(({ Icon, label, href }) => (
              <motion.a 
              href={href} key={label} target="_blank" aria-label={label} rel="noopener noreferrer" 
              variants={glowVariants}
              initial="initial"
              whileHover="hover"
              whileTap="tap"
              className='text-gray-300  transitions-colors duration-200 inline-flex items-center justify-center'  
               >
                <Icon />
              </motion.a>
            ))}
        ...
        <p className='text-xs text-gray-400  '>
          &copy; {new Date().getFullYear()} Rahul Kaushik. All rights reserved. 
        </p>
      </motion.div>
    </footer>
  )
